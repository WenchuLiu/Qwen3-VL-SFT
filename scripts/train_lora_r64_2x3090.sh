#!/usr/bin/env bash
# Single-node, two-GPU BF16 LoRA SFT. Defaults target Qwen3-VL-4B, not 8B.
# Run this script from the repository root; default paths are relative to it.
# Example: DATASET=data/coco/train_sft_10pct_1to4_11829.json bash scripts/train_lora_r64_2x3090.sh
set -euo pipefail
...
RUN_ID="${RUN_ID:-qwen3vl-4b-r64-2x3090-$(date +%Y%m%d-%H%M%S)}"
...
if count < 2:
    raise SystemExit(
        f"This preset needs two visible CUDA GPUs; found {count}. "
