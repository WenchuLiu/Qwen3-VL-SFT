...
# Evaluate the fixed 500-query-image COCO manifest with 1024 generated tokens.
...
OUTPUT="${OUTPUT:-${ROOT_DIR}/runs/coco-500-eval.json}"
SHOTS="${SHOTS:-0 1 2 4}"
...
if [[ ! -f "${SUPPORT_IMAGE_IDS}" && -f "${ROOT_DIR}/data/coco/train_image_ids_10pct.json" ]]; then
  SUPPORT_IMAGE_IDS="${ROOT_DIR}/data/coco/train_image_ids_10pct.json"
fi
